 ...
 */
class BasicSecurityHandler extends SecurityHandler{
/**
  ...
  handle(req) {
    try {
      ...
      if (user) {
        ...
        return SecurityHandler.error401(`Authorization error: "invalid name / password"`);
      }
    }
    catch (err) {
      return SecurityHandler.error401(`Authorization error: "invalid basic authentication"`);
    }
  }
}
